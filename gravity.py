...
while running:
    ...
    # Обновление горизонтальной скорости
    if moving_left:
        player_velocity[0] = -PLAYER_SPEED
    elif moving_right:
        player_velocity[0] = PLAYER_SPEED
    else:
        player_velocity[0] = 0  # Сбрасываем скорость, если не двигаемся
    ...
    pygame.draw.line(screen, BLUE,
                     ...
                     (player_pos[0] + PLAYER_SIZE, player_pos[1] + PLAYER_SIZE), 2)  # Диагональная линия

    pygame.draw.line(screen, BLUE,
                     (player_pos[0] + PLAYER_SIZE, player_pos[1]),
                     (player_pos[0], player_pos[1] + PLAYER_SIZE), 2)  # Обратная диагональ
